    ...
    <div className="flex flex-col h-full space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="font-display text-4xl uppercase tracking-tighter mb-2">
            {phase === 'game_over' && isFinal ? 'FINAL RESULTS' : 'ROUND RESULTS'}
          </h1>
      ...
      {(phase === 'backroom' || phase === 'game_over') && (
        <div className="bg-surface-low border border-outline-variant p-3 font-mono text-xs text-on-surface-variant">
          Backroom phase — no submissions accepted. Approach the organiser desk to negotiate deals.
        </div>
      )}
      {phase === 'game_over' && isFinal && rows[0] && (
        <div className="bg-surface-low border border-outline-variant p-3 font-mono text-sm text-primary">
          Winner: {rows[0].team_name}
        </div>
      )}
